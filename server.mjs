...
// Mock transport for the server to satisfy SDK connection requirements
const mockTransport = {
  sendMessage: async (message) => {
    // Mock transport that does nothing but satisfies the connection requirement
...
// Set the transport to satisfy SDK connection requirements
        ...
        try {
          ...
          const response = await axios.get(verifyUrl, {
            ...
            headers: {
              ...
              'User-Agent': 'eSewa-MCP-Server/1.0.0'
            }
          });
          ...
          if (error.code === 'ECONNREFUSED') {
            errorMessage = 'Connection refused - eSewa service may be unavailable';
          } else if (error.code === 'ETIMEDOUT') {
            errorMessage = 'Connection timeout - eSewa service is slow or unreachable';
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  ...
                  errorDetails,
                  note: "Make sure the transaction exists, credentials are correct, and eSewa service is accessible",
                  troubleshooting: [
                    "Check your internet connection",
                    "Verify eSewa service status",
                    "Check if using correct environment (sandbox/production)",
                    "Ensure transaction ID and amount are correct"
                  ]
                }, null, 2),
              },
            ],
...
// Standalone MCP endpoint that bypasses SDK connection requirements
app.post("/mcp", async (req, res) => {
  try {
    ...
    // Handle the request based on method type - completely bypass SDK
    let result;
    
    if (method === "tools/list") {
      // Return tools list directly from our implementation
      result = {
        jsonrpc: "2.0",
        id: id || 1,
        result: {
              ...
              result: {
                content: [{
                  ...
                  text: JSON.stringify({
                    ...
                    message: "Currently using SANDBOX mode. Call 'configure_credentials' to use production credentials."
                  }, null, 2)
                }]
              }
            };
          ...
          default:
            result = {
              ...
              result: {
                content: [{
                  type: "text",
                  text: `Tool '${name}' called with arguments: ${JSON.stringify(args, null, 2)}`
                }]
              }
            };
        }
